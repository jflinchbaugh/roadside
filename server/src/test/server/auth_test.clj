...
(deftest authfn-test
  (testing "authfn correctly identifies enabled/disabled users"
    (db/save-user {:xt/id "user1" :login "user1" :password (hashers/derive "pass") :enabled? true :site "test"})
    (db/save-user {:xt/id "user2" :login "user2" :password (hashers/derive "pass") :enabled? false :site "test"})
