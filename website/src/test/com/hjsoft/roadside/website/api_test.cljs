  ...
  (async done
    (testing "fetch-stands constructs correct URL and auth"
      (let [deps (make-mock-deps
                  :get (fn [url opts]
                         (is (= "api/stands" url))
                         (is (= {:username "user" :password "pass"} (:basic-auth opts)))
                         (is (= {:lat 1.0 :lon 2.0} (:query-params opts)))
                         (mock-http-response {:success true :body [{:id 1}]} )))]
        (go
      ...
      (let [stand {:name "New Stand"}
            deps (make-mock-deps
                  :post (fn [url opts]
                          ...
                          (mock-http-response {:success true :body {:id "new-id"}})))]
        (go
      ...
      (let [stand {:id "s123" :name "Updated"}
            deps (make-mock-deps
                  :put (fn [url opts]
                         ...
                         (is (= stand (:json-params opts)))
                         (mock-http-response {:success true :body stand})))]
        (go
  ...
  (async done
    (testing "geocode-address handles successful lookup"
      (let [deps (make-mock-deps
                  :get (fn [url opts]
                         (is (= "api/geocode" url))
                         (is (= "123 Main" (get-in opts [:query-params :q])))
                         (mock-http-response {:success true
                                              :body [{:lat "40.0" :lon "-76.0"}]} )))]
        (go
          (let [result (<! (sut/geocode-address "user" "pass" "123 Main" deps))]
            (is (:success result))
            (is (= 40.0 (:lat result)))
            (is (= -76.0 (:lng result)))
            (done)))))))
    ...
    (testing "reverse-geocode handles successful lookup"
      (let [deps (make-mock-deps
                  :get (fn [url opts]
                         ...
                         (mock-http-response {:success true :body {:address {:road "Main"}}} )))]
        (go
          (let [result (<! (sut/reverse-geocode "user" "pass" 40.0 -76.0 deps))]
            (is (:success result))
    ...
    (testing "register-user sends form params"
      (let [deps (make-mock-deps
                  :post (fn [url opts]
                          ...
                          (mock-http-response {:status 201 :body {:msg "ok"}})))]
        (go
