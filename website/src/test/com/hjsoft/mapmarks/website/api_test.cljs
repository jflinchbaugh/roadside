  ...
  (async done
         (testing "fetch-marks constructs correct URL and auth"
           (let [deps (make-mock-deps
             ...
             (go
               (let [result (<! (sut/fetch-marks "user" "pass" 1.0 2.0 "2026-03-21T12:00:00Z" deps))]
                 (is (:success result))
                 (is (= {:marks [{:id 1}]
                         :deleted-ids []
                         :new-sync "2026-03-21T13:00:00Z"}
  ...
  (async done
         (testing "fetch-marks handles HTTP error"
           (let [deps (make-mock-deps
             ...
             (go
               (let [result (<! (sut/fetch-marks "user" "pass" 1.0 2.0 nil deps))]
         ...
         (testing "create-mark sends POST with correct body"
           (let [mark {:name "New Mark"}
                 deps (make-mock-deps
                       :post (fn [url opts]
                               (is (= "api/marks" url))
                               (is (= mark (:json-params opts)))
                               (mock-http-response
                                {:success true
                                 :body {:id "new-id"}})))]
             (go
               (let [result (<! (sut/create-mark "user" "pass" mark deps))]
                 (is (:success result))
  ...
  (async done
         (testing "update-mark sends PUT to specific resource"
           (let [mark {:id "s123" :name "Updated"}
                 deps (make-mock-deps
                       :put (fn [url opts]
                              (is (= "api/marks/s123" url))
                              (is (= mark (:json-params opts)))
                              (mock-http-response {:success true
                                                   :body mark})))]
             (go
               (let [result (<! (sut/update-mark "user" "pass" mark deps))]
                 (is (:success result))
         ...
         (testing "delete-mark sends DELETE"
           (let [deps (make-mock-deps
                       :delete (fn [url opts]
                                 ...
                                 (mock-http-response {:success true})))]
             (go
               (let [result (<! (sut/delete-mark "user" "pass" "s123" deps))]
                 (is (:success result))
                 (done)))))))
